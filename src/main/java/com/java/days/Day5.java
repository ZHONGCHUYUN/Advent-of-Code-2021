    ...
    public int solution_part2() {
        ...
        for (List<Integer> coordinate: coordinates) {
            ...
            if (x1 == x2) {
                ...
                for (int x=Math.min(x1,x2), y=Math.min(y1,y2); x<=Math.max(x1,x2) && y<=Math.max(y1,y2); x++,y++) {
                    if (hashMap.containsKey(x + "," + y) && hashMap.get(x + "," + y) == 1) {
                        result ++ ;
                        hashMap.put(x + "," + y, 2);
                    } else {
                ...
                for (int x=Math.min(x1,x2), y=Math.max(y1,y2); x<=Math.max(x1,x2) && y>=Math.min(y1,y2); x++,y--) {
                    if (hashMap.containsKey(x + "," + y) && hashMap.get(x + "," + y) == 1) {
                        result ++ ;
                        hashMap.put(x + "," + y, 2);
                    } else {
